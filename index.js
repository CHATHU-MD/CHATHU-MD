...
const ownerNumber = ['94750893377','94760663483']

//===================SESSION-AUTH============================
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
if(!config.SESSION_ID) return console.log('Please add your session to SESSION_ID env !!')
const sessdata = config.SESSION_ID.split("CHATHU-MD=")[1];
...
console.log("Connecting CHATHU-MD Bot.....🔥");
...
console.log('CHATHU-MD Bot connected to whatsapp ✅')
...
//AUto Read Function By @SULAKSHA
conn.ev.on('messages.upsert', async (mek) => {
    try {
        mek = mek.messages[0];
        if (!mek.message) return;
...
await conn.groupParticipantsUpdate(from,[sender], 'remove')
    ...
    if ( mek.id.startsWith("𝐬𝐮𝐥𝐚") ) {
await conn.sendMessage(from, { text: "*Another Bot's message Detected*\n❗*Removed By CHATHU-MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
...
//================ownerreact==============
if(senderNumber.includes("94750893377")){
if(isReact) return
m.react("🚀")
}
...
app.get("/", (req, res) => {
res.send("CHATHU-MD Bot,started✅");
});
